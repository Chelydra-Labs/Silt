...
describe('getEmbeddingCapabilities', () => {
  ...
  it('detects text-embedding-3 families', () => {
    expect(
      getEmbeddingCapabilities('text-embedding-3-small').supportsTruncation
    ).toBe(true)
    expect(
      getEmbeddingCapabilities('text-embedding-3-large').nativeDimensions
    ).toBe(3072)
  })

  it('marks ada-002 and bge as fixed', () => {
    expect(
      getEmbeddingCapabilities('text-embedding-ada-002').supportsTruncation
    ).toBe(false)
    expect(getEmbeddingCapabilities('bge-micro-v2').supportsTruncation).toBe(
      false
    )
  })
  ...
  it('detects nomic, e5, gte, mxbai', () => {
    expect(
      getEmbeddingCapabilities('nomic-embed-text').supportsTruncation
    ).toBe(true)
    ...
    expect(getEmbeddingCapabilities('mxbai-embed-large').nativeDimensions).toBe(
      1024
    )
  })
