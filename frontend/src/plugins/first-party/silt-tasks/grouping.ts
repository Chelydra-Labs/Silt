...
/**
 * The universe of distinct non-empty values for a dimension, sorted the
 * same way binByDimension orders them. Used by the future sidebar to
 * derive filter/group pickers off the data; pure + deterministic so it
 * mirrors what the renderer will show.
 */
export function distinctValues(rows: TaskDetail[], dim: GroupBy): string[] {
  if (dim === 'none' || dim === 'dueDate') return []
  if (dim === 'status') {
    const set = new Set(rows.map((r) => r.status || 'TODO'))
    return [...set].sort((a, b) => {
