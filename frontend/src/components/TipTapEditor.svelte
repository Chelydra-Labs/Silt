<script lang="ts">
  ...
  // SearchPages may be generated by the backend lane after this frontend
  // change lands. The expected contract keeps this lane typed without a shim.
  const expectedPageBindings = AppBindings as typeof AppBindings & {
    SearchPages: (query: string, limit: number) => Promise<PageLinkItem[]>
    ResolvePageLink: (target: string) => Promise<PageLinkResolution>
  }
  ...
  function onPageLinkChange(ctx: PageLinkContext | null): void {
    ...
    pageLinkQueryTimer = setTimeout(async () => {
      pageLinkQueryTimer = null
      try {
        const request = expectedPageBindings.SearchPages(
          ctx.query.trim(),
          50
        ) as Promise<PageLinkItem[]> & {
          cancel?: () => Promise<void> | void
        }
  ...
  async function onPageLinkPick(item: PageLinkItem): Promise<void> {
    ...
    try {
      ...
      const inserted = await applyPageLinkSuggestion(
        editorInstance,
        item,
        expectedPageBindings.ResolvePageLink,
        alias
      )
      if (myId !== pageLinkQueryReqId) return
      if (!inserted)
