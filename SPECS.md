...
**Upgrade migration.** Vaults that already declared `created` or `aliases` as type-schema property names (before those keys became reserved) are repaired on vault open: the property is renamed to a free non-reserved name (`created` → `created_value`, `aliases` → `aliases_list`, with numeric suffixes on collision), matching page frontmatter keys for pages of that type are rewritten so values are preserved, and a one-time dismissible notice lists the renames. The reservation remains hard on save thereafter; ordinary schema edits still do not batch-rewrite pages.
