...
| Discovery | `silt mcp` prefers the OS-keyring-pinned endpoint (`Silt` / `mcp-local-endpoint`), then `mcp-endpoint.json` only when it matches the pin (or the pin is unavailable), then port 17887. The file alone cannot redirect the bearer. The endpoint file stores `{endpoint,pid}` under a cross-process lock; a second Silt instance will not clear or overwrite a live peer’s discovery record (or its keyring pin). Multi-instance desktop use is still unsupported as a product mode — the pin remains single-slot — but bind failure must not break the first instance’s `silt mcp` discovery. |
