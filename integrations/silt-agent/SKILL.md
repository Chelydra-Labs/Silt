...
3. **Confirm before writes.** Never call `create_page` or `update_blocks` unless the user explicitly asked to change the vault. Summarize the planned edit and wait for confirmation.
...
6. **Identity-preserving edits.** When updating a page, keep existing block `id` values whenever possible so links and history stay stable.

## Typical workflow

1. `list_notebooks` if you need structure.
2. `search_blocks` with a focused query (+ optional notebook/tag filters).
3. `read_page` or `read_blocks` for the hits you will use.
4. Answer with citations.
5. Only after confirmation: `create_page` or `update_blocks`.
