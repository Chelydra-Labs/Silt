...
3. **Confirm before writes.** Never call write tools (`create_page`, `append_to_page`, `insert_under_heading`, `create_task`, `update_blocks`, `set_page_property`, `set_page_type`, `restore_page_version`) unless the user explicitly asked to change the vault. Summarize the planned edit and wait for confirmation.
