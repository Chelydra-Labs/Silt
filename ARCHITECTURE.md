...
indent_block / unindent_block hotkeys (defaults Tab / Shift-Tab) are resolved
via resolveShortcut into SiltBlockKeymaps at editor creation. Visual nesting
uses data-depth on the outer NodeView root (outerNodeViewAttrs) so
`.ProseMirror > div[data-depth]` CSS applies. The cycle_view_layout hotkey is wired in App.svelte's
