...
- **Typed notes** — schema-driven note types live in `<vault>/.system/types/*.yaml`
  ...
  worker also emits `types:reprojection:progress` as a typed
  `TypesReprojectionProgressEvent` (`{state,processed,total}`, throttled:
  start, every ~25 pages / ~5%, done) so the type dashboard can show a
  non-blocking progress affordance, and `GetTypesReprojectionStatus` returns a
  typed `TypesReprojectionStatus` (`{active,processed,total}`) for a surface
  mounted mid-reprojection; `ReloadTypes` (manual schema refresh) drives a full
  pass and is the dashboard's user-facing Refresh action. **Type
